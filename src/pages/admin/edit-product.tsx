...
// ─────────────────────────────────────────────
// Outer page: fetches data, shows loading state,
// then mounts the form only once data is ready.
// ─────────────────────────────────────────────
export default function AdminEditProductPage() {
  ...
  const { data: productData, isLoading: productLoading } = useQuery({
    ...
    queryFn: () => getAdminProductByIdQueryFn(id!),
    enabled: !!id,
  });
  ...
  if (categoriesLoading || productLoading || !product) {
    return (
  ...
  // key={product._id} guarantees a fresh form instance per product,
  // so useForm's defaultValues are correct on first render.
  return (
    <EditProductForm
      key={product._id}
      product={product}
...
// ─────────────────────────────────────────────
// Inner form: only mounted once product + categories exist.
// ─────────────────────────────────────────────
function EditProductForm({
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["admin-product", product._id] });
